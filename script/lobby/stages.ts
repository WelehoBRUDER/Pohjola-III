...
class Stage {
  ...
  score: number;
  isBoss: boolean;
  constructor({ id, score, foes, isBoss }: StageObject) {
...
function createDungeonElement(dungeon: any): HTMLDivElement {
...
function createStages(stages: Stage[]) {
  ...
  stages.forEach((stage: Stage) => {
    ...
    if (player.completed_stages.includes(stage.id)) {
      ...
      dungeon = dungeons.find((dungeon: any) => dungeon.beat_stage_to_unlock === stage.id);
    }
