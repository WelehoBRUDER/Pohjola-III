...
var developerCommands = [
    {
        ...
        execute: function (args) {
            var command = args[0];
            if (command) {
                var commandObject = developerCommands.find(function (c) { return c.name === command; });
                if (commandObject) {
                ...
                var text_1 = "Available commands:";
                developerCommands.forEach(function (command) {
                    text_1 += "<br>" + command.name + ": " + command.description;
                });
                devConsole.commandHistory.push(text_1);
            }
        }
    },
    {
    ...
    },
    {
        ...
        execute: function (args) {
            ...
            var saveToLocalStorage = args[0] === "save";
    ...
    },
    {
        ...
        execute: function (args) {
            var item = args[0];
            var quantity = args[1] ? parseInt(args[1]) : 1;
            if (item) {
                // @ts-ignore
                var itemObject = items[item];
                if (itemObject) {
                    var newItem = new Item(itemObject);
    ...
    },
    {
        ...
        isCheat: true,
        execute: function (args) {
            var amount = args[0] ? parseInt(args[0]) : 100;
    ...
    },
    {
        ...
        isCheat: true,
        execute: function (args) {
            var xp = args[0] ? parseInt(args[0]) : 0;
            player.addXP(xp);
    ...
    },
    {
        ...
        isCheat: true,
        execute: function (args) {
            var sp = args[0] ? parseInt(args[0]) : 0;
