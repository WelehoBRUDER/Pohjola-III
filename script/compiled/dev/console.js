    ...
    if (e.key === "ArrowDown") {
        if (!listOpen) {
            ...
            if (devConsole.command >= devConsole.commandsHistory.length)
                devConsole.command = devConsole.commandHistory.length - 1;
            consoleInput.value = devConsole.commandsHistory[devConsole.command] || "";
        }
    }
    if (e.key === "Enter") {
...
function consoleIntellisense(options) {
    ...
    if (valueArr.length === 1 && (commandName.length > 0 || options?.showAll)) {
        commandOptions = options?.showAll
            ? developerCommands
            : developerCommands.filter((command) => command.name.startsWith(commandName));
    }
    ...
    if (commandOptions.length === 0)
        return (intellisenseData.active = false);
