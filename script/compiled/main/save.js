...
class SaveController {
    ...
    sortSaves() {
        this.saveSlots = this.saveSlots.sort((a, b) => b.lastSaved.getTime() - a.lastSaved.getTime());
    }
    getSaves(options) {
...
function saveScreen(menu = false) {
    ...
    <div class="line">|</div>
        ...
        <div class="line">|</div>
        <div class="ver">${game.getLocalizedString("version")}: <span class="${isOutOfDate ? "old" : "current"}">${gameVersionText(save.version)}</span></div>
      ...
      <div class="save-buttons">
        ${!menu
            ? `<button class="save-over ${(hardcore || hardcoreSave) && "disabled"}" onclick="saveController.saveOver('${save.id}')">${game.getLocalizedString("save")}</button>`
            : ""}
